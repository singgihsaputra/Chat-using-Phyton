...
class ChatClient(Frame):
  ...
  def initUI(self):
    ...
    path = 'C:/Python27/sister/foto.png'
    ...
    self.profil = ScrolledText(readChatGroup, bg="white", width=55, height=10, state=DISABLED)
    ...
    self.profil.grid(row=0,column=0,pady=5)
    self.receivedChats.grid(row=2,column=0,pady=5)
    self.friends.grid(row=2,column=1,pady=5)
  ...
  def listenClients(self):
    s = socket.socket()
    s.bind((""+self.serverIPVar.get().replace(' ',''),9999))
    s.listen(10)
    ...
    while 1:
         ...
         while True:
             ...
             f = open('file_'+self.clientIPVar.get().replace(' ','')+".jpg",'wb') #open in binary
             i=i+1
             print(i)
             l = 1
             while(l):
         ...
         try:
             ...
             path1 = 'file_'+self.clientIPVar.get().replace(' ','')+'.jpg'
             self.img1= ImageTk.PhotoImage(Image.open(path1))
             self.panel1 = Label(readChatGroup, image = self.img1 )
             self.panel1.pack(padx=0, pady=5, )
             update(initUI())
  ...
  def handleAddClient(self):
    ...
    try:
        s = socket.socket()
        s.connect((""+self.clientIPVar.get().replace(' ',''),9999))
        f = open("foto.jpg","rb")
        l = f.read(1024)
        while(l):
			s.send(l)
